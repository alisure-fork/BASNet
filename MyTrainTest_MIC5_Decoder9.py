...
class BASNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 2
        # label = self.salient_map_divide(cam_norm_up, obj_th=0.67, bg_th=0.33, more_obj=self.more_obj)  # 显著图划分
        # 3
        # label = self.salient_map_divide(cam_norm_up, obj_th=0.6, bg_th=0.4, more_obj=self.more_obj)  # 显著图划分
        # 3
        label = self.salient_map_divide(cam_norm_up, obj_th=0.5, bg_th=0.5, more_obj=self.more_obj)  # 显著图划分
...
class BASRunner(object):
    ...
    def all_loss_fusion(self, mic_1_out, mic_2_out, mic_3_out, mic_labels_1, mic_labels_2, mic_labels_3,
                        sod_1_sigmoid, sod_2_sigmoid, sod_3_sigmoid, sod_label):
    ...
    def train(self, save_epoch_freq=5, eval_epoch_freq=5, print_ite_num=100, update_epoch_freq=1):
        ...
        for epoch in range(0, self.epoch_num):
            ...
            all_loss_sod_1, all_loss_sod_2, all_loss_sod_3 = 0.0, 0.0, 0.0
            self.net.train()
            for i, (inputs, indexes) in enumerate(self.dataloader_usod):
                ...
                loss, loss_mic, loss_sod = self.all_loss_fusion(mic_target_1, mic_target_2, mic_target_3,
                                                                mic_labels_1, mic_labels_2, mic_labels_3,
                                                                sod_target_1, sod_target_2, sod_target_3, sod_label)
                loss.backward()
                self.optimizer.step()
                ...
                all_loss_sod_1 += loss_sod[0].item()
                all_loss_sod_2 += loss_sod[1].item()
                all_loss_sod_3 += loss_sod[2].item()
                if i % print_ite_num == 0:
                    Tools.print("[E:{:4d}/{:4d}, b:{:4d}/{:4d}] loss:{:.2f}/{:.2f} "
                                "mic 1:{:.2f}/{:.2f} mic 2:{:.2f}/{:.2f} mic 3:{:.2f}/{:.2f} "
                                "sod 1:{:.2f}/{:.2f} sod 2:{:.2f}/{:.2f} sod 3:{:.2f}/{:.2f}".format(
                        ...
                        all_loss_mic_3/(i+1), loss_mic[2].item(), all_loss_sod_1/(i+1), loss_sod[0].item(),
                        all_loss_sod_2/(i+1), loss_sod[1].item(), all_loss_sod_3/(i+1), loss_sod[2].item()))
                    pass
