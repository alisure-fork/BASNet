...
class BASNet(nn.Module):
    ...
    def __init__(self, n_channels, clustering_num_list=None, pretrained=True, has_mask=True, more_obj=False):
        ...
        self.decoder_3_c = ConvBlock(128, 64, has_relu=True)
        self.decoder_4_b = ResBlock(64, 64)  # 224
        self.decoder_4_out = nn.Conv2d(64, 1, 3, padding=1)
        self.decoder_4_c = ConvBlock(64, 64, has_relu=True)
        self.decoder_5_b = ResBlock(64, 64)  # 224
        self.decoder_5_out = nn.Conv2d(64, 1, 3, padding=1)
    ...
    def forward(self, x):
        ...
        d3_d4 = self.up_to_target(self.mic_up_2(self.decoder_3_c(d3)), e1) + e1  # 64 * 224 * 224
        ...
        d4 = self.decoder_4_b(d3_d4)  # 64 * 224 * 224
        d4_d5 = self.up_to_target(self.decoder_4_c(d4), e0) + e0  # 64 * 224 * 224
        d4_out = self.decoder_4_out(d4)  # 1 * 224 * 224
        d4_out_sigmoid = torch.sigmoid(d4_out)  # 1 * 224 * 224  # 小输出
        d4_out_up = d4_out  # 1 * 224 * 224
        d4_out_up_sigmoid = d4_out_sigmoid  # 1 * 224 * 224  # 大输出

        return_d4 = {
            "out": d4_out,
            "out_sigmoid": d4_out_sigmoid,
            "out_up": d4_out_up,
            "out_up_sigmoid": d4_out_up_sigmoid
        }

        d5 = self.decoder_5_b(d4_d5)  # 64 * 224 * 224
        d5_out = self.decoder_5_out(d5)  # 1 * 224 * 224
        d5_out_sigmoid = torch.sigmoid(d5_out)  # 1 * 224 * 224  # 小输出
        d5_out_up = d5_out  # 1 * 224 * 224
        d5_out_up_sigmoid = d5_out_sigmoid  # 1 * 224 * 224  # 大输出

        return_d5 = {
            "out": d5_out,
            "out_sigmoid": d5_out_sigmoid,
            "out_up": d5_out_up,
            "out_up_sigmoid": d5_out_up_sigmoid
        }

        return_m = {"m1": return_m1, "m2": return_m2, "m3": return_m3}
        return_d = {"label": return_d0, "d1": return_d1, "d2": return_d2,
                    "d3": return_d3, "d4": return_d4, "d5": return_d5}
    ...
    def train(self, save_epoch_freq=5, print_ite_num=100, update_epoch_freq=1):
        ...
        for epoch in range(0, self.epoch_num):
            ...
            for i, (inputs, indexes) in enumerate(self.dataloader_usod):
                ...
                # 1
                # target = return_d["d4"]["out_up_sigmoid"]
                # 2
                target = return_d["d5"]["out_up_sigmoid"]
...
if __name__ == '__main__':
    ...
    bas_runner = BASRunner(batch_size_train=6, has_mask=True, more_obj=False,
                           model_dir="./saved_models/my_train_mic5_decoder7_aug_mask_norm_5bce_d5")
